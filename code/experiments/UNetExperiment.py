...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: 1st dimension = bath size, 2nd dimension = class probability
            #         3rd dimension = y pixel, 4th dimension = z pixel
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                prediction = self.model(data)
                prediction_softmax = F.softmax(prediction, dim=1)
                loss = self.loss_function(prediction_softmax, target[:, 0, :, :])        #.squezz()  
