...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        for i in range(image.shape[0]):
            image[i,:,:] = image[i,:,:].astype(np.single)/np.max(image[i,:,:])
        ...
        # ANSWER: In calculating the performance metric like dice and jaccard coefficient, the data must be in integer type.    
